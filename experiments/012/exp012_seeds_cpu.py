"""Seed check (CPU lane): baseline random-tt at seeds 1-2, MC only.
(random-tt seeds 1-2 at chi_x=32 already exist in exp008 part B:
MC 40.1 / 68.0 — re-run here under the identical protocol for a clean
comparison anyway.)"""
import json
import sys
import time
...
for seed in (1, 2):
    res = TTStateReservoir(n_dims=8, beta=0.95, chi_w=8, chi_x=32,
                           chi_in=4, w_kind="random-tt", seed=seed)
    ...
    rec = {"part": "seeds", "w_kind": "random-tt", "chi_w": 8,
           "seed": seed, "runner": "cpu", "beta": 0.95, "chi_x": 32,
           "mc300": round(mc, 2),
