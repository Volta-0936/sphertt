"""Factorial completion: is the vanishing kron-sum gain a beta effect or
a depth effect?  Run kron-sum at (d8, beta=0.99, chi_x=48), seeds 0-2.
Reference (random-tt, same config, exp007/008): MC 113.6 / 70.4 / 103.1."""
import json
import sys
import time
...
for seed in (0, 1, 2):
    res = TTStateReservoir(n_dims=8, beta=0.99, chi_w=8, chi_x=48,
    ...
    mc = memory_capacity(X.astype(np.float64), u_mc, washout=300,
                         delays=300)
    rec = {"part": "factorial", "w_kind": "kron-sum", "seed": seed,
           "n_dims": 8, "beta": 0.99, "chi_x": 48,
           "mc300": round(mc, 2),
           "delta_bar": round(float(res.deltas_[300:].mean()), 5),
           "ms_per_step": round((time.time() - t0) / 4000 * 1e3, 1),
           "reference_random_tt": [113.6, 70.4, 103.1]}
